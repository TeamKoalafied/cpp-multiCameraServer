...
void threadedSystem::cameraSetup(){
    // start cameras
...
void threadedSystem::cargoGripThread(){
  ...
  while (true) {
    ...
    if (wideFovSink.GrabFrame(wideFovMat) == 0) {
      // Send the output the error.
      pipelineOutputCargo.NotifyError(wideFovSink.GetError());
...
void threadedSystem::hatchGripThread(){
  const int kWidth = 320;
  const int kHeight = 240;
  // std::vector<cs::VideoSource> cameras;
  // for (auto&& cameraConfig : cameraConfigs)
  ...
  while(true){
    ...
    if (wideFovSink.GrabFrame(wideFovMat) == 0) {
      // Send the output the error.
      pipelineOutputHatch.NotifyError(wideFovSink.GetError());
      // skip the rest of the current iteration
      continue;
    }
...
void threadedSystem::stripGripThread(){
  const int kWidth = 320;
  const int kHeight = 240;
  // std::vector<cs::VideoSource> cameras;
  // for (auto&& cameraConfig : cameraConfigs)
  ...
  while(true){
    ...
    if (wideFovSink.GrabFrame(wideFovMat) == 0) {
      // Send the output the error.
      pipelineOutputStrip.NotifyError(wideFovSink.GetError());
      // skip the rest of the current iteration
      continue;
    }
    ...
    std::cout << centreX << std::endl;
    std::cout << centreY << std::endl;
    // x_Angle = ((CentreX / 320)48-24);
    // std::cout << Angle offset is <<
...
void threadedSystem::crosshairsCroppedThread(){
  const int kWidth = 320;
  const int kHeight = 240;
  // std::vector<cs::VideoSource> cameras;
  // for (auto&& cameraConfig : cameraConfigs)
  ...
  while (true) {
    ...
    if (wideFovSink.GrabFrame(wideFovMat) == 0) {
      // Send the output the error.
      croppedOutput.NotifyError(wideFovSink.GetError());
      // skip the rest of the current iteration
      continue;
    }
...
int main(int argc, char* argv[]) {
  ...
  threadedSystem letsGo;
  letsGo.cameraSetup();

  // start image processing on camera 0 if present
  // WARNING: IF NO CAMERAS, THIS WILL NOT WORK, ORIGINAL if camera size over zero { RELIED ON ABOVE CODE SNIPPET
  std::thread t0 (&threadedSystem::crosshairsCroppedThread,&letsGo);
  std::thread t1 (&threadedSystem::cargoGripThread,&letsGo);
  std::thread t2 (&threadedSystem::hatchGripThread,&letsGo);
  std::thread t3 (&threadedSystem::stripGripThread,&letsGo);
